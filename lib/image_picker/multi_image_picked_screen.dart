...
class MultiImagePickedScreen extends StatefulWidget {
  ...
  const MultiImagePickedScreen({Key? key,required this.imageFiles}) : super(key: key);
...
class _MultiImagePickedScreenState extends State<MultiImagePickedScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Picked Images"),),
      body: Container(
        color: Colors.amberAccent,
        child: ListView.builder(
            itemCount: widget.imageFiles?.length,
            itemBuilder: (context, index){
              return Image.file(File(widget.imageFiles![index].path),height: 200,width: 200,);
            }),
      ),
    );
  }
